...
pub fn apply_linux_desktop_services(
    ...
    let mut state = LinuxDesktopServiceState {
...
pub fn start_desktop_event_forwarder<F>(
    state: &LinuxDesktopServiceState,
...
fn write_autostart_entry(entry: &AutostartEntry) -> Result<(), String> {
    ...
    let label = format!("dev.fenestra.{}", sanitize_id(&entry.id));
...
fn runtime_dir() -> Result<PathBuf, String> {
    if let Some(path) = env::var_os("XDG_RUNTIME_DIR") {
        return Ok(PathBuf::from(path).join("fenestra"));
    }
    Ok(home_dir()?.join("Library").join("Caches").join("fenestra"))
}
