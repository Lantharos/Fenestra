...
pub(crate) const OSR_TOKEN_ENV: &str = "MULLION_OSR_TOKEN";
...
fn runtime_ipc_dir(app_id: &str) -> io::Result<PathBuf> {
    ...
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        ...
        if path.is_absolute() {
            return Ok(path.join("mullion").join(sanitized));
        }
    }
...
mod tests {
    ...
    fn sanitize_app_id_keeps_safe_chars() {
        assert_eq!(sanitize_app_id("com.mullion.notes"), "com.mullion.notes");
