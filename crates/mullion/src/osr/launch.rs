...
pub(crate) fn require_app_id(config: &MullionWindowConfig) -> MullionResult<&str> {
    config
        .app_id
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| MullionError::CreationFailed {
            message: "Mullion requires a non-empty app_id (set via .app_id(...) or with_manifest)"
                .into(),
        })
}
...
pub(crate) struct OpenWindowContext {
    ...
    pub(crate) bridge: mullion_bridge::BridgeRegistry,
    pub(crate) security: mullion_bridge::ContentSecurity,
}
...
pub(crate) fn spawn_osr_host_child(
    ...
    config: &MullionWindowConfig,
    url: &str,
) -> MullionResult<std::process::Child> {
...
pub(crate) fn attach_open_window(
    ...
    let context = process
        ...
        .as_ref()
        .ok_or_else(|| MullionError::CreationFailed {
            message: "this Mullion process does not support open_window".into(),
        })?;
...
pub(crate) fn cef_osr_command(
    ...
    if !token_file.as_os_str().is_empty() {
        command.arg(format!("--mullion-osr-token-file={}", token_file.display()));
    }
    command
