...
#[cfg(target_os = "linux")]
use crate::osr_layer_host;
use crate::osr_transport::{IpcEndpoint, IpcListener, IpcStream};
use crate::{
...
fn overlay_texture_id(overlay_id: &str) -> String {
    if overlay_id == POPUP_OVERLAY_ID {
        ...
        format!("__mullion_guest_{overlay_id}")
    }
}
...
struct OsrNativeHost {
    ...
    child: Option<Child>,
    socket: Option<Arc<Mutex<IpcStream>>>,
...
impl OsrNativeHost {
    ...
    fn launch_child(&mut self) {
        ...
        let (endpoint, listener) = match IpcEndpoint::bind() {
        ...
        let authentication_token = match crate::osr_transport::authentication_token() {
            Ok(token) => token,
            Err(error) => {
    ...
    fn capture_guest(&self, browser_id: &str, request_id: &str, guest_id: &str) {
        ...
        self.send_control(&format!(
            "MULLION_BRIDGE_RESPONSE\t{browser_id}\t{request_id}\t{status}\t{payload}\n"
        ));
    }
    ...
    fn display_list(&self, width: f32, height: f32) -> DisplayList {
        ...
        if !self.config.transparent || uses_mullion_chrome(self.config.chrome) {
...
fn uses_mullion_chrome(chrome: MullionWindowChrome) -> bool {
    matches!(chrome, MullionWindowChrome::Mullion)
}
...
fn trace_host(config: &OsrHostConfig, stage: impl AsRef<str>) {
    let enabled = std::env::var(crate::MULLION_TRACE_ENV).is_ok_and(|value| {
        matches!(
    ...
    eprintln!(
        "mullion trace [{label}] osr-host pid={} {}",
        std::process::id(),
        stage.as_ref()
    );
}
...
fn platform_chrome(chrome: MullionWindowChrome) -> PlatformWindowChrome {
    match chrome {
