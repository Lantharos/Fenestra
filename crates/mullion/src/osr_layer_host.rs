...
pub(crate) fn run(config: OsrHostConfig) -> Result<(), String> {
    let mut shell_surface = config
        .shell_surface
        .clone()
        .ok_or_else(|| "missing Mullion shell surface options".to_string())?;
...
struct PopupSurface {
    ...
    scratch: Vec<u8>,
    mapped: bool,
    effect: Option<mullion_platform::WindowEffect>,
}
...
impl OsrLayerHost {
    ...
    fn handle_host_event(
        ...
        match event {
            ...
            LayerHostEvent::Message(OsrMessage::GuestHidden(_id)) => {
                ...
                // slot (shared with mullion.popup). GuestHidden clears that slot.
                self.close_popup(state);
            }
    ...
    fn ensure_child(&mut self) {
        ...
        let Ok(authentication_token) = crate::osr_transport::authentication_token() else {
            eprintln!("failed to secure Mullion layer OSR transport");
            return;
        };
        let Some(socket_path) =
            open_socket_reader(self.sender.clone(), authentication_token.clone())
        else {
            return;
        };

        let (width, height, scale) = self.content_size_for_cef();
        let endpoint = crate::osr_transport::IpcEndpoint::Unix(socket_path);
        ...
        let child = match command.spawn() {
            Ok(child) => child,
            Err(error) => {
                eprintln!("failed to launch Mullion layer OSR child: {error}");
                return;
            }
        };
    ...
    fn set_surface_margin(
        &mut self,
        margin: mullion_platform::ShellSurfaceMargin,
        state: &WindowState<()>,
    ) {
