...
pub(super) fn run_progress_window(
    ...
    match state.lock() {
        Ok(guard) => guard
            .done
            .clone()
            .unwrap_or_else(|| Err("Mullion setup did not complete".to_string())),
        Err(_) => Err("Mullion setup did not complete".to_string()),
    }
}
...
impl ApplicationHandler for ProgressApp {
    ...
    fn resumed(&mut self, event_loop: &dyn ActiveEventLoop) {
        ...
        let window: Arc<dyn Window> = match event_loop.create_window(attributes) {
            ...
            Err(error) => {
                eprintln!("failed to open Mullion setup window: {error}");
                event_loop.exit();
                return;
            }
        };
        let context = match Context::new(window.clone()) {
            Ok(context) => context,
            Err(error) => {
                eprintln!("failed to create Mullion setup context: {error}");
                event_loop.exit();
                return;
            }
        };
        let surface = match Surface::new(&context, window.clone()) {
            Ok(surface) => surface,
            Err(error) => {
                eprintln!("failed to create Mullion setup surface: {error}");
                event_loop.exit();
                return;
            }
        };
