...
use mullion_service::{AppManifest, prepare_machine_with_progress};
...
pub(crate) fn run_from_args(args: &[String]) -> bool {
    ...
    let result = ui::run_progress_window("Preparing Mullion", move |state, proxy| {
...
pub(crate) fn prepare(config: &MullionWindowConfig) -> MullionResult<()> {
    ...
    if resolve_runtime(&config.runtime).is_ok() {
        let report =
            mullion_service::adopt(register).map_err(|error| MullionError::CreationFailed {
                message: format!("failed to register with Mullion service: {error}"),
            })?;
