...
/// Generate the C++ header that embeds the canonical Mullion bridge script
/// (kept in `crates/mullion-bridge/src/web_bridge.js` and included as a
/// `&str` from `mullion_bridge::INSTALL_SCRIPT`). The C++ host includes
/// this header and the C++ raw string literal
/// `MULLION_BRIDGE_JS_RAW` becomes a drop-in replacement for the
/// hand-maintained host-side copies.
///
...
fn bridge_js_header() -> String {
    ...
    output.push_str(
        "// AUTO-GENERATED by mullion/src/host.rs from\n\
         // crates/mullion-bridge/src/web_bridge.js. Do not edit by hand.\n\
         #pragma once\n\
         constexpr const char* MULLION_BRIDGE_JS_RAW = R\"js(",
    );
...
impl HostBuildLock {
    fn acquire(runtime_dir: &Path) -> Result<Self, String> {
        let path = runtime_dir.join(".mullion-host-build.lock");
            ...
            match OpenOptions::new().write(true).create_new(true).open(&path) {
                ...
                Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                    ...
                    if started.elapsed() >= HOST_BUILD_LOCK_TIMEOUT {
                        return Err(format!(
                            "timed out waiting for Mullion CEF host build lock at {}",
                            path.display()
                        ));
                    }
