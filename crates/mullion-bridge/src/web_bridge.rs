//! Mullion bridge surface and canonical injected script.
//!
//! The script in `web_bridge.js` is the single source of truth for
//! `window.mullion` inside every Mullion page. The Rust helpers in this
//! module lets the CEF host build a fully-instantiated
//! install script for a given command allow-list, build the
//! `mullion://bridge/<id>?...` URL that page-side `invoke()` produces, and
//! parse that URL back into a [`BridgeRequest`] on the host side.

use std::collections::BTreeSet;

/// The canonical Mullion bridge script. The CEF host is
/// expected to set `window.__mullionBridgeCommands` to a JSON array of
/// allowed bridge command names before executing this script.
pub const INSTALL_SCRIPT: &str = include_str!("web_bridge.js");

/// Build the fully-instantiated Mullion bridge install script for a given
/// command allow-list. The returned string is safe to feed straight to
/// CEF's `ExecuteJavaScript`.
pub fn install_script(commands: &[&str]) -> String {
    ...
    let json = unique
        ...
        .collect::<Vec<_>>()
        .join(",");
    let prelude = format!("window.__mullionBridgeCommands=[{json}];");
    format!("{prelude}{INSTALL_SCRIPT}")
}

/// A bridge request parsed from a `mullion://bridge/<id>?...` URL produced
...
/// via `window.__mullionBridgeResolve(id, ok, payload)`.
...
/// The Mullion bridge scheme used for host-side dispatch.
pub const BRIDGE_SCHEME: &str = "mullion://bridge/";

/// The Mullion window-control scheme (`mullion://window/<action>?...`)
/// used by `window.mullion.window.show()` etc.
pub const WINDOW_SCHEME: &str = "mullion://window/";
...
/// Parse a `mullion://bridge/<id>?...` URL into a [`BridgeRequest`].
...
/// A window-control URL parsed from `mullion://window/<action>?...`.
...
mod tests {
    ...
    fn install_script_substitutes_command_list() {
        ...
        assert!(script.starts_with("window.__mullionBridgeCommands="));
        assert!(script.contains("\"notes.list\""));
        assert!(script.contains("\"vault.unlock\""));
        assert!(script.contains("mullion://bridge/"));
    }
    ...
    fn window_command_parses() {
        let parsed = WindowCommand::parse("mullion://window/show?at=1234-0.5").unwrap();
