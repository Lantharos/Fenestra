...
const SERVICE_REPO: &str = "Lantharos/Mullion";
...
pub fn ensure_service_executable(
    ...
    on_progress(PrepareProgress {
        ...
        message: "Downloading Mullion service".to_string(),
        fraction: Some(0.02),
    });
    ...
    on_progress(PrepareProgress {
        stage: PrepareStage::Service,
        message: "Mullion service ready".to_string(),
        fraction: Some(0.08),
    });
    Ok(destination)
}
...
fn service_download_url() -> String {
    if let Ok(url) = std::env::var("MULLION_SERVICE_URL")
        && !url.trim().is_empty()
    {
        return url;
    }
...
fn service_asset_name() -> String {
    ...
    if cfg!(target_os = "windows") {
        format!("mullion-service-{os}-{arch}.exe")
    } else {
        format!("mullion-service-{os}-{arch}")
    }
}
...
fn download_file(
    ...
    if let Some(stderr) = child.stderr.take() {
        ...
        for line in reader.split(b'\r').flatten() {
            if let Some(percent) = parse_curl_percent(&line) {
                on_progress(PrepareProgress {
                    stage: PrepareStage::Service,
                    message: format!("Downloading Mullion service ({percent:.0}%)"),
    ...
    if status.success() {
        ...
        Err(ServiceError::Update(format!(
            "failed to download Mullion service from {url}; publish a release asset or set MULLION_SERVICE_URL / MULLION_SERVICE_PATH"
        )))
    }
}
...
mod tests {
    ...
    fn asset_names_are_platform_shaped() {
        ...
        assert!(name.starts_with("mullion-service-"));
