...
// `WebView2Process::take_desktop_events` / the bridge forwarder.
...
pub struct WindowsDesktopServiceState {
    events: EventQueue,
...
impl std::fmt::Debug for WindowsDesktopServiceState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WindowsDesktopServiceState")
            .field(
                "queued_events",
...
impl WindowsDesktopServiceState {
...
pub fn apply_windows_desktop_services(
...
) -> Result<WindowsDesktopServiceState, String> {
    let events = Arc::new(Mutex::new(Vec::new()));
    let mut state = WindowsDesktopServiceState {
...
pub fn start_desktop_event_forwarder<F>(
    state: &WindowsDesktopServiceState,
...
fn register_native_messaging_host(host: &NativeMessagingHost) -> Result<(), String> {
    ...
    let manifest_dir = local_app_data()?.join("fenestra").join("native-messaging");
...
impl SingleInstanceGuard {
    fn acquire(
        ...
        if already {
            ...
            return Err("another Fenestra instance is already running".to_string());
        }
        let _ = handle;
...
fn instance_port_path(name: &str) -> Result<PathBuf, String> {
    let dir = local_app_data()?.join("fenestra").join("instances");
...
#[allow(dead_code)]
fn unused_menu_item_type(_: &TrayMenuItem) {}
