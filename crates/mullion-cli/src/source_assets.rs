...
pub fn metadata(source: &Path) -> SourceMetadata {
    let mullion = source.join("Mullion.toml");
    let mut metadata = SourceMetadata::default();
    if mullion.exists() {
        merge_mullion_metadata(source, &mullion, &mut metadata);
    }
...
fn web_config(source: &Path) -> Option<WebConfig> {
    ...
    read_web_config(source, &source.join("Mullion.toml"), &mut config);
...
fn merge_mullion_metadata(source: &Path, path: &Path, metadata: &mut SourceMetadata) {
    let Ok(value) = read_toml(path) else {
        return;
    };
