...
pub(super) fn resolve_app(source: &Path, overrides: ConfigOverrides) -> Result<BundleApp, String> {
    ...
    let mullion = read_mullion_file(&source_dir)?;
    ...
    let web = resolve_web(&source_dir, &mullion.web, &overrides)?;
    ...
    let id = overrides
        .id
        .or(mullion.app.id)
        .unwrap_or_else(|| format!("dev.mullion.{}", sanitize_id(&name)));
    let version = overrides
        .version
        .or(mullion.app.version)
        .unwrap_or_else(|| {
    ...
    Ok(BundleApp {
        ...
        version,
        icon,
        mime_types: mullion.app.mime_types,
        cargo_manifest,
        source_dir,
        cargo_package,
        web,
...
fn read_mullion_file(source_dir: &Path) -> Result<MullionFile, String> {
    let path = source_dir.join("Mullion.toml");
    if !path.exists() {
        return Ok(MullionFile::default());
    }
...
mod tests {
    ...
    fn remote_only_web_config_does_not_require_local_assets() {
        let source = PathBuf::from("/tmp/mullion-remote-only-config-test");
        let web = resolve_web(
            &source,
            &WebSection {
    ...
    fn explicit_local_assets_are_kept_for_site_backed_apps() {
        let source = PathBuf::from("/tmp/mullion-site-backed-config-test");
        let web = resolve_web(
            &source,
            &WebSection {
