...
fn stage_appimage(
    ...
    stage_resources(
        app,
        format,
        &app_dir.join("usr/share/mullion").join(&app.id),
    )?;
    Ok(app_dir)
}
...
fn stage_unix_root(
    ...
    let resources = app_dir.join("usr/share/mullion").join(&app.id);
    fs::create_dir_all(&bin_dir).map_err(|error| error.to_string())?;
...
fn linux_icon_path(app: &BundleApp, format: BundleFormat) -> Option<String> {
    ...
    {
        return Some(format!(
            "/usr/share/mullion/{}/icons/scalable/apps/{}.svg",
            app.id, app.id
        ));
    }
    Some(format!(
        "/usr/share/mullion/{}/icons/512x512/apps/{}.png",
        app.id, app.id
    ))
}
