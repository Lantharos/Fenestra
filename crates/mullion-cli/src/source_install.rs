...
fn register_app(app: &SourceApp, desktop: bool) -> Result<(), String> {
    ...
    mullion_service::MullionService::default()
        .register(mullion_service::AppManifest {
            id: app.id.clone(),
            name: app.name.clone(),
...
fn apps_root() -> Result<PathBuf, String> {
    Ok(data_home()?.join("mullion/apps"))
}
