...
pub fn run_dev(options: DevOptions) -> Result<ExitCode, String> {
    ...
    if let Some(dev_frontend) = &project.frontend {
        ...
        if options.web_only {
            eprintln!("mullion: web-only mode; Ctrl+C to stop");
            let code = wait_child(frontend.take())?;
            return Ok(code);
        }
    } else {
        eprintln!("mullion: static web assets (no frontend dev server)");
    }

    eprintln!(
        "mullion: cargo run --manifest-path {}",
...
fn ensure_node_modules(frontend: &web_detect::DevFrontend) -> Result<(), String> {
    ...
    eprintln!(
        "mullion: installing JS deps with {} in {}",
...
fn run_cargo(
    ...
    command.env("MULLION_SKIP_DEV_COMMAND", "1");
    if let Some(frontend) = frontend {
        command.env("MULLION_DEV_URL", &frontend.url);
    }
    let status = command
        .status()
