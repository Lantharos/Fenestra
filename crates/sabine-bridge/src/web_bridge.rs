//! Sabine bridge surface and canonical injected script.
//!
//! The script in `web_bridge.js` is the single source of truth for
//! `window.sabine` inside every Sabine page. The Rust helpers in this
//! module lets the CEF host build a fully-instantiated
//! install script for a given command allow-list, build the
//! `sabine://bridge/<id>?...` URL that page-side `invoke()` produces, and
//! parse that URL back into a [`BridgeRequest`] on the host side.

use std::collections::BTreeSet;

/// The canonical Sabine bridge script. The CEF host is
/// expected to set `window.__sabineBridgeCommands` to a JSON array of
/// allowed bridge command names before executing this script.
pub const INSTALL_SCRIPT: &str = include_str!("web_bridge.js");

/// Build the fully-instantiated Sabine bridge install script for a given
...
pub fn install_script(commands: &[&str]) -> String {
    ...
    let json = unique
        ...
        .collect::<Vec<_>>()
        .join(",");
    let prelude = format!("window.__sabineBridgeCommands=[{json}];");
    format!("{prelude}{INSTALL_SCRIPT}")
}

/// A bridge request parsed from a `sabine://bridge/<id>?...` URL produced
...
/// via `window.__sabineBridgeResolve(id, ok, payload)`.
...
/// The Sabine bridge scheme used for host-side dispatch.
pub const BRIDGE_SCHEME: &str = "sabine://bridge/";

/// The Sabine window-control scheme (`sabine://window/<action>?...`)
/// used by `window.sabine.window.show()` etc.
pub const WINDOW_SCHEME: &str = "sabine://window/";
...
/// Parse a `sabine://bridge/<id>?...` URL into a [`BridgeRequest`].
...
/// A window-control URL parsed from `sabine://window/<action>?...`.
...
mod tests {
    ...
    fn install_script_substitutes_command_list() {
        ...
        assert!(script.starts_with("window.__sabineBridgeCommands="));
        assert!(script.contains("\"notes.list\""));
        assert!(script.contains("\"vault.unlock\""));
        assert!(script.contains("sabine://bridge/"));
    }
    ...
    fn window_command_parses() {
        let parsed = WindowCommand::parse("sabine://window/show?at=1234-0.5").unwrap();
