...
/** @typedef {import("./index.d.ts").MullionApi} MullionApi */
...
function requireGuestApi() {
  ...
  if (!guest?.create) {
    throw new Error(
      "Mullion guests are not enabled for this window. Register guest bridge commands on the host.",
    );
  }
  return guest;
}
...
export function isAvailable() {
  return Boolean(globalThis.window?.mullion?.bridge?.__native);
}
...
/**
 * Handle for a guest surface created through the Mullion bridge.
 */
export class Guest {
  /** @param {string} id */
  constructor(id) {
      ...
      typeof result === "string"
        ...
        : result && typeof result === "object" && "id" in result && result.id
          ...
          : options.id
            ...
            : null;
    if (!id) {
      throw new Error("Mullion guest.create did not return an id");
    }
    return new Guest(id);
  }
...
export const popup = {
  ...
  open(options = {}) {
    ...
    if (!api?.open) {
      throw new Error("Mullion popups are not enabled for this window.");
    }
